...
def lastseen(conn, target, channel, params):
  ...
  for result in results[::-1]:
    conn.msg(target, result['nick']+"!"+result["identhost"]+" was last seen ")
...
#        self.looper.start(finterval) #todo: synchronize looper with this message
      ...
      if len(params)==0:
        ...
        if params[0].lower()=="chess":
          self.msg(target,
               ...
               "\n"
               ".cm or .chessmove [game number] <origin square> <destination square>\n"
               ...
               " if game number is omitted, the last game you created, viewed, or"
               " moved in is assumed.\n"
               "Ex.: .cm e2 e4\n"
    ...
    def chessmove(self, u, target, params):
      ...
      if len(params)==3:
        ...
        chessid = cursor.fetchone()["cchessid"]
        if chessid is None:
          self.msg(target, "Please include the game number parameter.")
          return
      else:
        self.msg(target, usage)
        return
