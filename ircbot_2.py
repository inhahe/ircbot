...
import time, sys, os.path, re, random, string, StringIO, weakref
...
versionname = "PyBot Python IRC bot by inhahe using Python 2.5 + Twisted IRC."
versionnum = "0.9.9"
versionenv = "Windows XP Pro SP2 (stolen)"
class network: pass
networks = []
#n = network()
#n.name = "freenode"
...
class bot(irc.IRCClient):
    ...
    def __init__(self, *args, **kwargs):
      global botinstances
      botinstances += [weakref.proxy(self)]
      self.channels = {}
      self.commandqueue = []
#      self.cmdtimes = []
      self.penalty = 0
    ...
    def privmsg(self, user, channel, msg):
        ...
        if channel.lower() not in self.channels:
          ...
          if fw == "aq" or fw==".aq":
            ...
            if cursor.rowcount:
              self.msg(channel, "Quote #%s deleted." % num)
            else:
...
#          del self.channels[channel.lower()].backbuffer[0]
    ...
    def action(self, user, channel, msg):
        ...
        logger.log("<%s> <%s> * %s %s" % (self.net, channel, user, msg))
        updateseen(nick, ident+host, self.net, channel)
        self.channels[channel.lower()].backbuffer.append(chanmsg(type="action",
               time=time.time(),
               text=msg, nick=nick, ident=ident, host=host,
               userid=getid(ident, host), username=getuname(ident, host)))
    ...
    def __del__(self):
      global botinstances
      botinstances.remove(weakref.proxy(self))
...
class LogBotFactory(protocol.ClientFactory):
    ...
    def clientConnectionLost(self, connector, reason):
        ...
        if reconnect:
          if connecttimes:
            while connecttimes[0] < time.now() - 60:
              connecttimes.pop(0)
          if len(connecttimes) > 5:
            connector.connect()
    ...
    def clientConnectionFailed(self, connector, reason):
        ...
        self.connecdict[self.hostcycle]
