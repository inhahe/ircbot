...
n.onconnect = lambda c: c.msg("nickserv", "identify common") 
networks.append(n)
n = network()
n.name = "dalnet"
n.joins = ["#spirituality,#freethinkers"]
...
def cplace(x1,y1,x2=None,y2=None):
  ...
  if x2!=None: a+=chr(ord('a')+x2)+str(8-y2)
  return a
...
#  cursor.execute("begin")
  cursor.execute("insert into %s (%s) values (%s)" % (table,
   fix(",".join([str(x[0]) for x in dict.items() if x[1] != None])),
   ",".join(["'%s'" % fix(str(x)) for x in dict.values() if x != None])))
#  cursor.execute("end")
...
#        self.looper.start(finterval) #todo: synchronize looper with this message
    ...
    def chesslist(self, u, target, params, unf):
      if len(params)==1:
        ...
        d = cursor.fetchall()
        if unf: d = [x for x in d if x["forfeit"]==None and x["won"]==None and
    ...
    def chesscopy(self, u, target, params):
      ...
      if len(params)>1:
        ...
        if not g:
          ...
          return
      if g["stalemate"] or g["won"]!=None or g["forfeit"] != None or g["draw"]:
          ...
          if p2y2==0:
            def finishpawn(self, user, channel, msg, chessid=chessid,
              ...
              board = board[:p2y*8+p2x] + [piece, piece.lower()][color] + \
                   board[p2y*8+p2x+1:]
              self.chessmove2(target, chessid, turn, board, cplace(p1x,p1y,p2x,p2y)+piece)
    ...
    def chessmove2(self, target, chessid, turn, board, move):
      ...
      self.showboard(target, chessid)
      n = [x.nick for x in users.values() if x.userid == [d["wuserid"], d["buserid"]][not turn]]
      if len(n):
        nick = n[0]
    ...
    def chessmove(self, u, target, params):
      ...
      self.chessmove2(target, chessid, turn, board, cplace(p1x, p1y, p2x, p2y))
        ...
        if channel.lower() not in self.channels:
          ...
          if nick and nick != "NickServ" and nick != "MemoServ":
            if fw in ["aq","rq",".aq",".rq", "dq",".dq"]:
              ...
              try:
                ...
                self.msg(nick, "Confuscius say: You must first log in before you log out.")
                return
          ...
          if fw == "aq" or fw==".aq":
            ...
            try:
              ...
              self.msg(channel, ".rq requires a quote number.")
              return
