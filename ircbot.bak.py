...
#help["base"] = """This command converts a number from one base to another."
#Usage:
#  !base <number> [base to convert from] <base to convert to>
#  if [base to convert from] is omitted, base 10 is assumed."""
    ...
    def help(self, target, params):
      if len(params)==0:
        self.msg(target, "Usage: .help [topic]\n"
              "Help topics: math, chess, quotes, accounts")
      elif len(params)==1:
        if params[0].lower()=="chess":
          ...
          "any arbitrary string of letters and numbers as a variable\n"
          "Lists are encased in square brackets ([]).\n"
          "A slice is written as [[args]] right after an expression and returns the component of the expression described by args. The first arg acts on the expression, the second arg acts on the result of the first, etc. A value of 0 returns a symbol for the head of the expression, a positive integer returns that component of the expression (as if the expression was a 1-based list) and a negative number returns the component that number from the end of the expression.\n"
          ".factor <number>\n"
          "Factors ")
    ...
    def privmsg(self, user, channel, msg):
        ...
        if channel.lower() not in self.channels:
          ...
          if fw == "aq" or fw==".aq":
            ...
            self.help(nick, msg.lower().split()[1:])
            self.msg(channel, "%s: I sent you private messages." % nick)
          elif fw == "sb":
